 ...
 *
 * Modified from {@link org.jetbrains.plugins.gradle.util.GradleAttachSourcesProvider}
 */
final class GradleAttachSourcesProvider implements AttachSourcesProvider {

  @Override
  public @NotNull Collection<AttachSourcesAction> getActions(@NotNull List<LibraryOrderEntry> orderEntries,
...
//        return GradleBundle.message("gradle.action.download.sources");
        return GradleBundle.message("gradle.action.download.sources") + "<Fixup>";
        // PATCH END
      }
      ...
      @Override
      public @NotNull ActionCallback perform(@NotNull List<LibraryOrderEntry> orderEntriesContainingFile) {
        Map<LibraryOrderEntry, Module> gradleModules = getGradleModules(orderEntries);
  ...
  static @NotNull String getSourcesArtifactNotation(@NotNull String artifactCoordinates,
    ...
    // PATCH BEGIN
    // force remove "aar" extension to enable "jar" sources fallback
    groupNameVersionCoordinates = groupNameVersionCoordinates.replaceAll("@aar", "");
    // PATCH END
